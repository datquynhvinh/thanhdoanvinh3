...
<?php
    if(isset($_POST['delete'])) {
        $id = $_GET['id'];
        if($id != null) {
            $sql = "delete from `lich-cong-tac` where id=$id";
                        ...
                        <div class="col">
                            <div class="card">
                                <div class="card-header">
                                    <a class="btn btn-primary" href="add.php" role="button"><i class="fas fa-plus"></i>Thêm lịch công tác</a>
                                </div>
                                <div class="card-body">
                                    <table id="datatable1" class="display" style="width:100%">
                                        ...
                                        <?php
                                            $i = 1; 
                                            $sql = mysqli_query($con,'SELECT * FROM `lich-cong-tac` ORDER BY id ASC');
                                            while($row = mysqli_fetch_array($sql)){?>
                                            <tr>
                                                <td><?php echo $i++ ?></td>
                                                <td class="admin_new_title"><?php echo $row['ngay'] ?> - <?php echo $row['gio'] ?></td>
                                                <td class="admin_new_content"><?php echo $row['noidung'] ?></td>
                                                <td class="admin_new_content"><?php echo $row['diadiem'] ?></td>
