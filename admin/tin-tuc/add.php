<?php
	...
	if(isset($_POST['add'])) {
		...
		if ($qr) {
		    // Bởi vì còn nội dung chưa render nên không thể sử dụng location
									...
									<form class="row needs-validation" novalidate method="POST" enctype="multipart/form-data" onsubmit="return validateForm()">
												...
												<select class="form-select" aria-label="Default select example" name="author">
													<?php
														...
														while ($row3 = mysqli_fetch_assoc($result3)) {
															echo "<option value='".$row3['id']."'>" . $row3["firstname"] . ' ' . $row3["lastname"] . "</option>";
														}
													?>
												</select>
											</div>
										...
										<div class="col-md-12">
											<div class="item">
												...
												<textarea class="form-control tinymce" name="content" id="exampleFormControlTextarea1" rows="3" required=""></textarea>
												<div class="invalid-feedback">
													Không được để trống phần này
												</div>
											</div>
